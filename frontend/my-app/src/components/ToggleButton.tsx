...
interface ToggleButtonProps {
  label: string;
  postEndpoint: string;
  system: 'System1' | 'System2' | 'Sync' | 'System3';
  stateKey: 'motorState' | 'reverseDirection' | 'syncState';
  onStateChange: (stateKey: 'motorState' | 'reverseDirection' | 'syncState', state: boolean) => void;
}
...
const ToggleButton: React.FC<ToggleButtonProps> = ({ label, postEndpoint, system, stateKey, onStateChange }) => {
  ...
  const handleToggle = () => {
    ...
    setState(newState);
    onStateChange(stateKey, newState);
    postToggleState(system, stateKey, newState);
  };

  const postToggleState = async (system: 'System1' | 'System2' | 'Sync' | 'System3', stateKey: 'motorState' | 'reverseDirection' | 'syncState', stateValue: boolean) => {
    try {
      let postData;
